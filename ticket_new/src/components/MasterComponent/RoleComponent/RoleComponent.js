...
import { getData,putDeleteData } from "../../../services/MastersService/RoleService";
...
function RoleComponent({location}) {
    ...
    const columns = [
                        { name: 'Id', selector: row => row.counter, sortable: true, },
                    ...
                    <div className="row clearfix g-3">
                        <div className="col-xl-12 col-lg-12 col-md-12 flex-column">
                            ...
                            <div className="card">
                                ...
                                border-bottom-0">
                                    <h2 className="mb-0 fw-bold ">Role Master</h2>
                                    <Link to="Role/Create/"  className="btn btn-dark btn-sm btn-set-task w-sm-100">
            ...
            <select className="form-control form-control-sm" 
                ...
                {data.map(function(item, i){
                    if(props.defaultValue && props.defaultValue==item.id){
                        return <option key={i} value={item.id} selected>{item.role}</option>
                    }else{
                        return <option key={i} value={item.id}>{item.role}</option>
                    }
